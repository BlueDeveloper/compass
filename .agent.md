...
2. **디바이스 방향 감지**
   ...
   **안정화 최적화 (v2.0)**:
     - **EMA (Exponential Moving Average)**: α=0.25, 이전 WMA 대비 더 빠른 반응
     - **mod360 함수**: 0~360 경계 처리 개선
     - **angleDiff 함수**: 최단 거리 각도 차이 계산 (-180~180)
     - 작은 변화 무시 (Threshold): 1.5도 이하 변화는 업데이트 안 함
     - Throttling: 100ms 간격으로 센서 데이터 처리
...
const smoothHeadingEMA = (newHeading: number) => {
  const ALPHA = 0.25; // 평활화 계수 (0.2~0.3 권장)
...
// 3. AbsoluteOrientationSensor (Android)
...
sensor.onreading = () => {
  ...
  if (Math.abs(angleDiff(smoothed, lastHeading)) < 1.5) return;
  setHeading(smoothed);
};

// 4. Throttling (100ms 간격)
// 5. CSS transition (0.5s ease-out)
```
...
- `ALPHA` (0.25): EMA 평활화 계수
  ...
  - 높을수록 (0.3~0.4): 빠른 반응이지만 떨림 가능
- `THROTTLE_MS` (100ms): 센서 업데이트 주기
  - 높을수록: 배터리 절약, CPU 부하 감소
  - 낮을수록: 빠른 반응
- `CHANGE_THRESHOLD` (1.5도): 무시할 최소 변화량
  - 높을수록: 안정적이지만 덜 민감
  - 낮을수록: 민감하지만 떨림 가능
- CSS transition (0.5s): 애니메이션 속도
  - 시각적 부드러움 조절
