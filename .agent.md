...
### v2.5 (Balanced & Stable) - 2026-02-07
**균형잡힌 나침반 + 안정적인 레이더**

- ✅ **나침반 모드 균형 조정**
  - ALPHA: 0.35 → 0.25 (빠름과 안정성 중간)
  - THROTTLE: 50ms → 100ms (초당 10회, 적절한 업데이트)
  - CHANGE_THRESHOLD: 0.5° → 1.5° (적절한 민감도)
  - CSS transition: 0.2s → 0.3s (부드러운 애니메이션)
  - **결과**: 너무 민감하지도, 너무 느리지도 않은 균형

- ✅ **레이더 모드 안정화**
  - heading 평활화 (ALPHA=0.3)
  - bearing (방위각) 평활화 (ALPHA=0.3)
  - distance (거리) 평활화 (ALPHA=0.3)
  - **결과**: 목표 지점이 안정적으로 고정됨
  - 떨림 현상 제거

**파라미터 비교:**
| 항목 | v2.3 Fast | v2.5 Balanced |
|------|-----------|---------------|
| ALPHA | 0.35 | 0.25 |
| Throttle | 50ms | 100ms |
| Threshold | 0.5° | 1.5° |
...
2. **디바이스 방향 감지**
   ...
   **안정화 최적화 (v2.5 Balanced)**:
     - **Single EMA**: α=0.25 (균형)
     - **Outlier Rejection**: 60도 이상 급변만 무시
     - **Warmup Period**: 초기 10샘플은 즉시 수용
     - **mod360 함수**: 0~360 경계 처리 개선
     - **angleDiff 함수**: 최단 거리 각도 차이 계산 (-180~180)
     - 작은 변화 감지 (Threshold): 1.5도 이상 변화 반영 (균형)
     - Throttling: 100ms 간격으로 센서 데이터 처리 (균형)
     - 부드러운 애니메이션: 0.3s ease-out transition
...
4. **UI 구현**
   ...
   **레이더 모드 (`/radar`):**
   ...
   - 자동/수동 줌 컨트롤
   - 나침반 모드 전환 버튼
   - **안정화**: heading, bearing, distance 모두 EMA 적용 (α=0.3)
   - **결과**: 목표 지점 떨림 없이 안정적
...
**안정화 파라미터 조정 (v2.5):**
- `ALPHA` (0.25): EMA 평활화 계수
  - 낮을수록 (0.1~0.2): 부드럽지만 느린 반응
  - 높을수록 (0.4~0.6): 매우 빠른 반응, 약간 떨림
  - **현재 (0.25)**: 균형잡힌 반응 속도 + 안정성 ⭐
- `OUTLIER_THRESHOLD` (60도): 급격한 변화 무시 임계값
  - 센서 오류 판정 기준
- `THROTTLE_MS` (100ms): 센서 업데이트 주기
  - 높을수록: 배터리 절약, 더 안정적
  - 낮을수록: 빠른 반응
  - **현재 (100ms)**: 초당 10회 업데이트 (균형) ⭐
- `CHANGE_THRESHOLD` (1.5도): 반영할 최소 변화량
  - 높을수록: 안정적이지만 덜 민감
  - 낮을수록: 매우 민감, 작은 움직임 감지
  - **현재 (1.5도)**: 적절한 민감도 (균형) ⭐
- `WARMUP_SAMPLES` (10개): 초기 안정화 샘플 수
  - 초기 방향 감지 속도 조절
- CSS transition (0.3s): 애니메이션 속도
  - **현재 (0.3s)**: 부드러운 시각 피드백 ⭐
